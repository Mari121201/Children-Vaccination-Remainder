...
try {
    ...
    //check button clicks and show patients
    ...
    if (isset($_POST['set'])) {
        ...
        $setTimeQuery = "update message_details set appointment_time='$setTime' where message_id='$msgID'";
        mysqli_query($connection, $setTimeQuery);
        echo "<script>alert('Appointment date set successfully');
            window.onload=function(){appointment();}</script>";
    }

    //setcancel button clicks
             ...
             margin:40px; ">
                ...
                if ($chooseCheck > 0) {
                    ...
                    while ($chooseRow = mysqli_fetch_assoc($chooseResult)) {
                        ...
                        $appointment_time = $chooseRow['appointment_time'];
                        $date = '0000-00-00';
                        $time = '00:00:00';
                        ...
                        if (mysqli_num_rows($queryCheckResult) > 0) {
                            ...
                            if ($appointment === $date && $appointment_time === $time) {
                                $count = 1;
        ...
        <form id="showSetDate" name="setDate" method="post" action="">
            <div id="setappointmentdate">
                <div id="setappointment-align">
                    ...
                    <center><input style="width:100px; height:25px; margin-top:20px;" type="time" name="settime"></center>
                </div>
            </div>
            <div id="setButton">
        ...
        <form id="showpatients" name="showpatients" method="post" action="">
            ...
            <div id="showChoose">
                ...
                if (mysqli_num_rows($mResult)) {
                    ...
                    echo "<tr><th>Patient Name</th><th>Phone number</th><th>Appointment date</th><th>Appointment Time</th>";
                    while ($mRow = mysqli_fetch_assoc($mResult)) {
                        ...
                        $mAppTime = $mRow['appointment_time'];
                        $mTime=date('h:i:s A',strtotime($mAppTime));
                        if ($mApp === $currentDate) {
                            ...
                            if (mysqli_num_rows($pResult) > 0) {
                                ...
                                echo "<tr style='height: 25px;font-weight: bold;padding: 10px;text-align: right;'><td>$pName</td><td>$pContact</td><td>$mApp</td><td>$mTime</td></tr>";
                            }
                        }
                    }
                    echo "</table>";
